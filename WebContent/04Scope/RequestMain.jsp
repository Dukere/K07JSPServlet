...
request.setAttribute("requestMember1", new MemberDTO("Nakja", "1234", "성강사", null));
...
<body>
	...
	<ul>
		<li>
		Integer타입 : <%=request.getAttribute("requestNumber") %>
		</li>
		<li>
		String타입 : <%=request.getAttribute("requestString") %>
		</li>
		<%
		MemberDTO member = 
	...
	<%
		request.removeAttribute("NoKeyValue");//키 없으나 에러 X
	...
	<ul>
		...
		<li>MemberDTO타입2 : 아이디 : <%=member.getId() %>,
							비번 : <%=member.getPass() %>,
							이름 : <%=member.getName() %>,
							가입일 : <%=member.getRegidate() %>
		</li>
