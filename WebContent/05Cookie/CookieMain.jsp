...
<body>
	...
	*/
	...
	2.쿠키의 적용경로 설정 : 쿠키 설정 시 쿠키가 적용될 경로를 설정한다.
		아래의 설정은 컨텍스트 루트 경로를 사용하므로 웹사이트 전체에서
		사용 가능한 쿠키를 생성하게 된다.
	*/
	...
	/* 
	3. 쿠키의 유효시간 설정(초단위)
		: 유효시간을 설정하지 않을 경우 웹브라우저를 닫을 때 자동으로
		삭제된다. 그 외에는 설정한 시간만큼 유지된다.
	*/
	cookie.setMaxAge(3600); //60*60 1시간
	
	/* 
	4.응답헤더에 쿠키를 추가하여 클라이언트로 전송한다. 이 때 클라이언트 측
		PC에 쿠키가 생성된다.
	*/
	response.addCookie(cookie);
	%>
	<!-- 
	쿠키는 첫 실행 시에는 출력되지 않는다. 이유는 클라 측으로
	응답헤더를 통해 전송되어 생성은 되었으나, 서버측으로 다시 응답헤더를
	통해 전송해야지만 확인이 가능하기 때문이다.
	즉, 새로고침하거나 다른 페이지로 요청이 있을 때
	쿠키를 확인할 수 있다.
	 -->
